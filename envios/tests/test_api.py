...
class TestAutenticacion:
    ...
    def test_token_personalizado_incluye_datos_empleado(self, api_client):
        user = UserFactory(username="jwt_user")
        ...
        assert access["username"] == user.username
        assert access["email"] == user.email
        assert access["empleado_id"] == empleado.id
        assert access["empleado_cod"] == "EMP-JWT"
...
@pytest.mark.parametrize(
    ...
    [
        "encomienda-fbv-list",
        "encomienda-apiview-list",
        "encomienda-mixin-list",
        "encomienda-generic-list",
    ],
)
def test_endpoints_de_fundamentos_drf_responden_200(url_name, auth_client):
    response = auth_client.get(reverse(url_name, kwargs={"version": "v1"}))
    assert response.status_code == status.HTTP_200_OK
...
class TestCrearEncomienda:
    ...
    def test_remitente_igual_destinatario_devuelve_400(self):
        ...
        assert "destinatario" in detail or "non_field_errors" in detail
...
class TestAccionesPersonalizadas:
    def setup_method(self):
        ...
        self.enc_retraso = EncomiendaFactory(
            estado="TR", empleado_registro=self.empleado,
        )
